...
You'll see the responsive dashboard that adapts to your terminal size:

**Standard Layout (120-198 columns wide):**
```
┌─ FORGE Control Panel ──────────────────────────────────────────────────┐
│                                                                         │
│  ┌─ Workers ──────┐  ┌─ Subscriptions ────┐                           │
│  │ sonnet-alpha   │  │ Claude:  [████] 328/500│                        │
│  │ opus-beta      │  │ ChatGPT: [██░] 12/40  │                        │
│  │ haiku-gamma    │  │ Reset: 16d 23h        │                        │
│  └────────────────┘  └─────────────────────┘                           │
│                                                                         │
│  ┌─ Tasks ─────────┐  ┌─ Activity Log ────┐                           │
│  │ bd-abc  [P0]   │  │ Worker spawned...  │                           │
│  │ bd-def  [P1]   │  │ Task completed...  │                           │
│  │ bd-ghi  [P2]   │  │ Optimizing costs...│                           │
│  └─────────────────┘  └────────────────────┘                           │
│                                                                         │
│  [Press : for chat | ? for help | C for theme | q to quit]               │
└─────────────────────────────────────────────────────────────────────────┘
```

**Ultra-Wide Layout (199+ columns):**
The dashboard expands to a 3-column layout showing all 6 panels simultaneously:
- **Left Column**: Workers + Subscriptions
- **Middle Column**: Tasks + Activity Log
- **Right Column**: Cost Breakdown + Quick Actions
...
FORGE supports 4 beautiful color themes that persist across sessions:

### Available Themes

| Theme | Description | Best For |
|-------|-------------|----------|
| **Default** | Classic cyan/blue color scheme | Standard terminal usage |
| **Dark** | Enhanced contrast with lighter colors | Dim environments |
| **Light** | Dark text on light background | Bright environments |
| **Cyberpunk** | Neon magenta/cyan aesthetic | Retro-futuristic style |

### Switching Themes

```bash
# In the dashboard, press:
C  # Cycle to next theme

# Or set theme directly via chat
...
Your theme choice is automatically saved to `~/.forge/theme.toml`:
...
The theme loads automatically on next launch.

### Custom Theme Colors

Each theme defines colors for:

- **Headers** - Panel titles and focused borders
- **Hotkeys** - Keyboard shortcut hints
- **Status** - Healthy (green), Warning (yellow), Error (red)
- **Actions** - Spawn (green), Kill (red), Refresh (cyan), View (blue), Config (yellow)
- **Budget Alerts** - Color scales from green → yellow → red based on usage percentage
...
┌─ FORGE Control Panel ──────────────────────┐
│  Workers (cyan border)                     │
│  ● sonnet-alpha (green status indicator)   │
└────────────────────────────────────────────┘
```
...
## Subscriptions & Budget Tracking

FORGE helps you maximize use-or-lose API subscriptions before falling back to pay-per-token pricing.

### Subscription Display

The Subscriptions panel shows:
...
│  Claude (Anthropic)                      │
│  ┌─────────────────────────────────┐    │
│  │ Used: 328,500 / 500,000 (66%)    │    │
│  │ ▓▓▓▓▓▓▓░░░░░░░░░░░░░░░░░░░░    │    │
│  │ Reset in: 16d 23h 42m            │    │
│  └─────────────────────────────────┘    │
│                                          │
│  ChatGPT (OpenAI)                        │
│  ┌─────────────────────────────────┐    │
│  │ Used: 12,340 / 40,000 (31%)     │    │
│  │ ▓▓▓░░░░░░░░░░░░░░░░░░░░░░░░░░  │    │
│  │ Reset in: 4d 12h 18m            │    │
│  └─────────────────────────────────┘    │
│                                          │
...
|------------|-------|---------|
| 0-69% | Green | Healthy usage |
| 70-89% | Yellow | Approaching limit |
| 90-99% | Light Red | Near exhaustion |
| 100%+ | Red | Over limit |

### Subscription Configuration

Configure your subscriptions in `~/.forge/config.yaml`:

```yaml
subscriptions:
  claude:
    provider: "anthropic"
    limit: 500000  # Total tokens per billing period
    period: "monthly"  # monthly, daily, weekly
    reset_day: 1  # Day of month for reset
    priority: 1  # Use first (lower = higher priority)

  chatgpt:
    provider: "openai"
    limit: 40000
    period: "daily"
    reset_hour: 0  # UTC hour for reset
    priority: 2
  ...
  glm:
    ...
    limit: 100000
    period: "weekly"
    reset_weekday: 1  # 0=Sunday, 1=Monday, etc.
    priority: 3
```

### Cost Optimization via Chat

```bash
# Check subscription usage
: Show subscription status
: Which subscription is closest to limit?

# Optimize worker routing
: Route tasks to maximize Claude subscription
: Use GLM for low-priority tasks
...
FORGE automatically tracks subscription resets and shows countdown timers:

- **Monthly**: Days + hours until next month reset
- **Daily**: Hours + minutes until UTC midnight reset
- **Weekly**: Days + hours until next Monday/Specified day reset
