...
//! forge-core's StatusWatcher to provide real-time updates.
...
impl WorkerData {
    ...
    pub fn format_activity_log(&self) -> String {
        ...
        // Sort workers by last_activity (most recent first)
        let mut workers: Vec<_> = self.workers.values().collect();
        workers.sort_by(|a, b| {
            b.last_activity
                .cmp(&a.last_activity)
...
impl DataManager {
    ...
    /// Poll for updates from the StatusWatcher.
    ///
