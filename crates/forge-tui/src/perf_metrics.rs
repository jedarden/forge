...
impl PerfMetrics {
    ...
    pub fn record_frame(&mut self, event_loop_us: u64, render_us: u64) {
        ...
        if event_loop_us > 16_667 {
            self.add_alert(
                PerfAlertType::SlowEventLoop,
                format!(
                    "Event loop latency {}ms exceeds 16ms target",
                    event_loop_us / 1000
                ),
                event_loop_us,
            );
    ...
    pub fn update_memory(&mut self, rss_bytes: u64) {
        ...
        if rss_bytes > 500_000_000 {
            ...
            if rss_bytes > prev_high {
                self.add_alert(
                    PerfAlertType::HighMemory,
                    format!("Memory usage is {}MB", rss_bytes / 1_000_000),
                    rss_bytes,
                );
            }
        }
    ...
    pub fn current_fps(&self) -> f64 {
        ...
        if elapsed > 0.0 {
            (self.frame_timestamps.len() as f64 - 1.0) / elapsed
        } else {
            0.0
        }
    }
